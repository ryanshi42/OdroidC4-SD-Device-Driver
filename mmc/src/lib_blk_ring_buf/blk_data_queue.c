...
blk_data_queue_result_t blk_data_queue_init(
    ...
    if (queue == NULL) {
        return ERR_NULL_BLK_DATA_QUEUE;
    }
    ...
    if (MAX_NUM_BLK_DATA_BUFS < queue->num_data_bufs + 1) {
     ...
     * performance reasons. */
    queue->num_unused_slots = (MAX_NUM_BLK_DATA_BUFS - queue->num_data_bufs);
    ...
    for (size_t i = 0; i < queue->num_data_bufs; i++) {
        ...
        blk_data_queue_result_t res = blk_data_queue_enqueue(
                queue,
                &data_buf
        );
        if (res != OK_BLK_DATA_QUEUE) {
            return res;
        }
    }
    return OK_BLK_DATA_QUEUE;
}
...
blk_data_queue_result_t blk_data_queue_capacity(
    ...
    if (ret_val == NULL) {
        return ERR_NULL_RET_VAL_PTR_PASSED_TO_BLK_DATA_QUEUE_FN;
    }
...
blk_data_queue_result_t blk_data_queue_enqueue(
     ...
     * enqueue. */
    bool is_full = false;
    blk_data_queue_result_t const res = blk_data_queue_is_full(queue, &is_full);
    if (res != OK_BLK_DATA_QUEUE) {
        return res;
    }
    if (is_full) {
        return ERR_BLK_DATA_QUEUE_FULL;
    }
    ...
    queue->tail_idx = (queue->tail_idx + 1) % MAX_NUM_BLK_DATA_BUFS;
    return OK_BLK_DATA_QUEUE;
}
...
blk_data_queue_result_t blk_data_queue_dequeue(
     ...
     * dequeue. */
    bool is_empty = false;
    blk_data_queue_result_t const res = blk_data_queue_is_empty(queue, &is_empty);
    if (res != OK_BLK_DATA_QUEUE) {
        return res;
    }
    if (is_empty) {
        return ERR_BLK_DATA_QUEUE_EMPTY;
    }
    ...
    queue->head_idx = (queue->head_idx + 1) % MAX_NUM_BLK_DATA_BUFS;
    return OK_BLK_DATA_QUEUE;
}
